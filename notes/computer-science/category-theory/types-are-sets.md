...
이상적인 상황에서, 타입은 집합이고 함수는 집합과 집합을 잇는 사상이다.

어떤 함수 `fn f(arg: X) -> Y`가 있을 때 타입 `X`, `Y`를 각각 정의역과 공역으로,
함수 `f`의 결과를 치역으로 생각할 수 있다:
...
아니면 결과가 나오기 전에 컴퓨터가 폭발되어 사라질 수도 있다.


따라서 끝나지 않는 작업(*non-terminating computation*)을 표현하기 위하여
*bottom*이라는 값이 도입되었다. $\bot$으로 표기하며, 모든 타입은 이를 값으로 갖는다.
`bool` 타입은 `true`, `false`, `⊥`을 원소로 취한다. 아래의 코드는 정상적으로 컴파일된다.
`true`나 `false`를 반환하지 않으므로 `⊥`을 반환한다고 볼 수 있다.
...
$\bot$의 존재로 프로그래밍 언어의 타입은 엄밀히 말하여 집합은 아니다. 그러나
그 외의 모든 것들은 집합과 유사하기 때문에 집합으로 생각하여도 괜찮다.
...
프로그램이 옳은지 분석하고 검증하는 것은 어려운 일이다. 프로그램을 타입의
...
그 외의 언어들 역시 `Nothing`, `Never`, `Void` 등의 이름으로 공타입을
정의하고 있다.

루비에서는 [`bot`][ruby-bot]이라는 이름으로 정의하고 있다. *subtype of all of the types*라고
설명하는데, 공집합 역시 모든 집합의 부분집합이므로 같은 성질을 공유한다.

[rust-never]: https://doc.rust-lang.org/std/primitive.never.html
[ruby-bot]: https://github.com/ruby/rbs/blob/master/docs/syntax.md#base-types
...
루비에서는 굉장히 많이 쓰인다. 루비의 `true`, `false`, `nil` 등의 값은
`TrueClass`, `FalseClass`, `NilClass`의 유일한 값이다. 따라서 이들은 단위 타입이다.
...
단위 타입은 사실 암묵적으로 정말 많이 쓰이고 있다. 아래의 함수 `hello_world`는
아무 인자를 취하지 않고 `String`을 반환하고 있다. 아무것도 아닌 것을 인자로
취하는 공타입과는 구별된다.
...
그런데 정말 아무것도 취하지 않고 `String`을 반환한다면 사실상 `String`과
...
단위 타입을 이용함으로써 아무것도 아님을 표현할 수 있게 되었다.

어떤 타입 `T`를 받아 단위 타입을 반환하는 함수는 딱 하나만 존재한다.
...
함수가 생기게 된다. 예를 들어 `() -> bool`인 함수는 아래의 두 함수가 있다.
